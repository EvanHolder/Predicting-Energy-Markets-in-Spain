...
### Modeling Same-day Price <a class="anchor" id="Same-day-Price"></a>
All the weather data in this analysis is live - data that was measured at that specific moment in time. It's not inuitive that weather data from today would help predict the price tomorrow as the weather often changes day to day very quickly. I attempted to find historical weather forecast data but was unsucessful in finding readily accessible, free data.  So while it is no surprise that most weather features appear unhelpful in predicting the price of tomorrow, I did develop a short analysis which tries to model *today's price* with *today's weather*.  Even though it would seem today's weather would have an impact on today's price, the models were just as noisy and no more informative.  See the notebook [here](https://github.com/EvanHolder/Predicting-Energy-Markets-in-Spain/blob/main/notebooks/Same-Day-Price.ipynb).
...
<p style="text-align:center;"><img src="https://github.com/EvanHolder/Predicting-Energy-Markets-in-Spain/blob/main/images/final_metrics.jpg" style="width:434px;height:100px"/></p>
